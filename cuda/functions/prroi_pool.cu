...
__global__ void prroi_pool_forward_cuda_kernel(
    const int nthreads, const void *input_, const void *rois_, void *output_,
...
__global__ void prroi_pool_backward_cuda_kernel(
    const int nthreads, const void *grad_output_, const void *rois_, void *grad_input_,
    const int pooled_height, const int pooled_width, const T spatial_scale,
    const int channels, const int height, const int width) {
...
__global__ void prroi_pool_coor_backward_cuda_kernel(
    const int nthreads, const void *output_, const void *grad_output_, const void *input_,
    const void *rois_, void *grad_rois_, const int pooled_height,
...
diopiError_t diopiPrroiPool(diopiContextHandle_t ctx, diopiTensorHandle_t input_,
...
diopiError_t
diopiPrroiPoolCoorBackward(diopiContextHandle_t ctx, diopiTensorHandle_t output_,
                           diopiTensorHandle_t grad_output_,
                           diopiTensorHandle_t input_, diopiTensorHandle_t rois_,
                           diopiTensorHandle_t grad_rois_, int64_t pooled_height,
                           int64_t pooled_width, float spatial_scale) {
  auto output = impl::cuda::makeTensor(output_);
  auto grad_output = impl::cuda::makeTensor(grad_output_);
  auto input = impl::cuda::makeTensor(input_);
