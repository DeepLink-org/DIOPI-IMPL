...
__device__ inline float cross(Point o, Point a, Point b) { return (a.x - o.x) * (b.y - o.y) - (b.x - o.x) * (a.y - o.y); }

__device__ inline float dis(Point a, Point b) { return (a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y); }
...
template <typename T>
__global__ void min_area_polygons_cuda_kernel(const int ex_n_boxes, const void *ex_boxes_, void *minbox_) {
    const T *ex_boxes = static_cast<const T *>(ex_boxes_);
    T *minbox = static_cast<T *>(minbox_);
...
diopiError_t diopiMinAreaPolygonsMmcv(diopiContextHandle_t ctx, diopiTensorHandle_t polygons_, diopiConstTensorHandle_t pointsets_) {
