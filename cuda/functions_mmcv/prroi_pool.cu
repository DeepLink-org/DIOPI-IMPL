...
template <typename T>
__device__ static T PrRoIPoolingInterpolation(const T *data, const T h, const T w, const int height, const int width) {
    T retVal = 0.0f;
    int h1 = floorf(h);
    int w1 = floorf(w);
    retVal += PrRoIPoolingGetData(data, h1, w1, height, width) * PrRoIPoolingGetCoeff(h - T(h1), w - T(w1));
    h1 = floorf(h) + 1;
    w1 = floorf(w);
    retVal += PrRoIPoolingGetData(data, h1, w1, height, width) * PrRoIPoolingGetCoeff(h - T(h1), w - T(w1));
    h1 = floorf(h);
...
template <typename T>
__device__ static T PrRoIPoolingMatCalculation(const T *this_data, const int s_h, const int s_w, const int e_h, const int e_w, const T y0, const T x0,
                                               const T y1, const T x1, const int h0, const int w0) {
    ...
    lim_alpha = x1 - T(s_w);
    lim_beta = y1 - T(s_h);
    tmp = (lim_alpha - 0.5f * lim_alpha * lim_alpha - alpha + 0.5f * alpha * alpha) * (lim_beta - 0.5f * lim_beta * lim_beta - beta + 0.5f * beta * beta);
...
template <typename T>
__device__ static void PrRoIPoolingMatDistributeDiff(T *diff, const T top_diff, const int s_h, const int s_w, const int e_h, const int e_w, const T y0,
                                                     const T x0, const T y1, const T x1, const int h0, const int w0) {
    T alpha, beta, lim_alpha, lim_beta, tmp;
...
template <typename T>
__global__ void prroi_pool_forward_cuda_kernel(const int nthreads, const void *input_, const void *rois_, void *output_, const int pooled_height,
                                               const int pooled_width, const T spatial_scale, const int channels, const int height, const int width) {
    ...
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        ...
        const T *this_data = input + (roi_batch_ind * channels + c) * height * width;
        ...
        for (int bin_x = start_x; bin_x < end_x; ++bin_x)
            for (int bin_y = start_y; bin_y < end_y; ++bin_y)
                sum_out += PrRoIPoolingMatCalculation(this_data,
                                                      bin_y,
                                                      bin_x,
                                                      bin_y + 1,
                                                      bin_x + 1,
                                                      max(bin_y1, T(bin_y)),
                                                      max(bin_x1, T(bin_x)),
                                                      min(bin_y2, T(bin_y) + 1.0f),
                                                      min(bin_x2, T(bin_x + 1.0f)),
                                                      height,
                                                      width);
...
template <typename T>
__global__ void prroi_pool_backward_cuda_kernel(const int nthreads, const void *grad_output_, const void *rois_, void *grad_input_, const int pooled_height,
                                                const int pooled_width, const T spatial_scale, const int channels, const int height, const int width) {
    ...
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        ...
        T *this_data_grad = grad_input + (roi_batch_ind * channels + c) * height * width;
        ...
        for (int bin_x = start_x; bin_x < end_x; ++bin_x)
            for (int bin_y = start_y; bin_y < end_y; ++bin_y)
                PrRoIPoolingMatDistributeDiff(this_data_grad,
                                              sum_out,
                                              bin_y,
                                              bin_x,
                                              bin_y + 1,
...
template <typename T>
__global__ void prroi_pool_coor_backward_cuda_kernel(const int nthreads, const void *output_, const void *grad_output_, const void *input_, const void *rois_,
                                                     void *grad_rois_, const int pooled_height, const int pooled_width, const T spatial_scale,
                                                     const int channels, const int height, const int width) {
    const T *output = static_cast<const T *>(output_);
    const T *grad_output = static_cast<const T *>(grad_output_);
    const T *input = static_cast<const T *>(input_);
    const T *rois = static_cast<const T *>(rois_);
    T *grad_rois = static_cast<T *>(grad_rois_);
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        // (n, c, ph, pw) is an element in the pooled output
        int pw = index % pooled_width;
        int ph = (index / pooled_width) % pooled_height;
        ...
        for (int bin_y = start_y; bin_y < end_y; ++bin_y) {
            grad_x1_y += PrRoIPoolingSingleCoorIntegral(max(bin_y1, T(bin_y)) - bin_y,
                                                        ...
                                                        PrRoIPoolingInterpolation(this_input_data, static_cast<float>(bin_y), bin_x1, height, width),
                                                        PrRoIPoolingInterpolation(this_input_data, static_cast<float>(bin_y + 1), bin_x1, height, width));

            grad_x2_y += PrRoIPoolingSingleCoorIntegral(max(bin_y1, T(bin_y)) - bin_y,
                                                        min(bin_y2, T(bin_y + 1)) - bin_y,
                                                        PrRoIPoolingInterpolation(this_input_data, static_cast<float>(bin_y), bin_x2, height, width),
                                                        PrRoIPoolingInterpolation(this_input_data, static_cast<float>(bin_y + 1), bin_x2, height, width));
        }

        for (int bin_x = start_x; bin_x < end_x; ++bin_x) {
            grad_x_y1 += PrRoIPoolingSingleCoorIntegral(max(bin_x1, T(bin_x)) - bin_x,
                                                        min(bin_x2, T(bin_x + 1)) - bin_x,
                                                        PrRoIPoolingInterpolation(this_input_data, bin_y1, static_cast<float>(bin_x), height, width),
                                                        PrRoIPoolingInterpolation(this_input_data, bin_y1, static_cast<float>(bin_x + 1), height, width));

            grad_x_y2 += PrRoIPoolingSingleCoorIntegral(max(bin_x1, T(bin_x)) - bin_x,
                                                        min(bin_x2, T(bin_x + 1)) - bin_x,
                                                        PrRoIPoolingInterpolation(this_input_data, bin_y2, static_cast<float>(bin_x), height, width),
                                                        PrRoIPoolingInterpolation(this_input_data, bin_y2, static_cast<float>(bin_x + 1), height, width));
        }
        ...
        // (index, x1, y1, x2, y2)
        this_rois_grad[0] = 0;
        atomicAdd(this_rois_grad + 1, (partial_x1 * (1.0f - T(pw) / pooled_width) + partial_x2 * (1.0f - T(pw + 1) / pooled_width)) * output_grad_val);
        atomicAdd(this_rois_grad + 2, (partial_y1 * (1.0f - T(ph) / pooled_height) + partial_y2 * (1.0f - T(ph + 1) / pooled_height)) * output_grad_val);
        atomicAdd(this_rois_grad + 3, (partial_x2 * T(pw + 1) / pooled_width + partial_x1 * T(pw) / pooled_width) * output_grad_val);
        atomicAdd(this_rois_grad + 4, (partial_y2 * T(ph + 1) / pooled_height + partial_y1 * T(ph) / pooled_height) * output_grad_val);
    }
}
...
diopiError_t diopiPrroiPool(diopiContextHandle_t ctx, diopiTensorHandle_t output_,diopiTensorHandle_t input_,
...
diopiError_t diopiPrroiPoolbackward(diopiContextHandle_t ctx,
    ...
    auto stream = impl::cuda::getStream(ctx);
    impl::cuda::prroi_pool_backward_cuda_kernel<float><<<GET_BLOCKS(output_size), THREADS_PER_BLOCK, 0, stream>>>(output_size,
                                                                                                                  grad_output.data(),
                                                                                                                  rois.data(),
                                                                                                                  grad_input.data(),
                                                                                                                  pooled_height,
...
diopiError_t
diopiPrroiPoolCoorBackward(diopiContextHandle_t ctx, diopiTensorHandle_t grad_rois_,
    ...
    auto input = impl::cuda::makeTensor(input_);
    auto rois = impl::cuda::makeTensor(rois_);
    auto grad_rois = impl::cuda::makeTensor(grad_rois_);
