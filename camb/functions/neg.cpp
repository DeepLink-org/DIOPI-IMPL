...
extern "C" DIOPI_API diopiError_t diopiNeg(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t input) {
    ...
    std::cout << "trInput:" << trInput.dtype() << "\ttrOutTmp:" << trOutTmp.dtype() << std::endl;
    DIOPI_CALLCNNL(cnnlNegTensor(handle, descInput.get(), trInput.data(), descOutTmp.get(), trOutTmp.data()));
...
extern "C" DIOPI_API diopiError_t diopiNegInp(diopiContextHandle_t ctx, diopiTensorHandle_t input) {
    std::cout << "INP" << std::endl;
    diopiNeg(ctx, input, input);
    return diopiSuccess;
}
