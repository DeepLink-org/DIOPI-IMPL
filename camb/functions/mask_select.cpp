...
diopiError_t nonzeroCount(diopiContextHandle_t ctx, DiopiTensor input_tensor,
                          DiopiTensor *num_true);

DIOPI_API diopiError_t diopiMaskedSelect(diopiContextHandle_t ctx,
  ...
  std::set<diopiDtype_t> mask_dtypes{diopi_dtype_bool, diopi_dtype_float16,
                                     diopi_dtype_float32};
  ...
  if (mask_tensor.dtype() != diopi_dtype_bool) {
    ...
    std::set<diopiDtype_t> input_dtypes{diopi_dtype_bool,   diopi_dtype_int8,
                                        diopi_dtype_uint8,  diopi_dtype_int16,
  ...
  auto temp_output_tensor =
      requiresTensor(ctx, input_numel, input_tensor.dtype());
  ...
  DIOPI_CALLCNNL(cnnlGetMaskedWorkspaceSize(
      handle, masked_mode, input_desc.get(), mask_desc.get(), nullptr,
  ...
  DIOPI_CALLCNNL(cnnlMasked_v3(
      handle, masked_mode, input_desc.get(), input_tensor.data(),
      mask_desc.get(), mask_tensor.data(), nullptr, nullptr, workspace,
      workspace_size, out_desc.get(), temp_output_tensor.data(),
      static_cast<uint32_t *>(num_true.data())));

  syncStreamInCtx(ctx);
  int64_t num_true_host = 0;
  cnrtMemcpy(&num_true_host, num_true.data(), sizeof(num_true.dtype()),
             CNRT_MEM_TRANS_DIR_DEV2HOST);
  std::vector<int64_t> output_shape(1, num_true_host);
  auto output_tensor =
      requiresTensor(ctx, output_shape, temp_output_tensor.dtype());

  DIOPI_CALL(diopiSlice(ctx, diopiTensorHandle_t(output_tensor),
                        diopiTensorHandle_t(temp_output_tensor), 0, 0,
                        num_true_host, 1));
