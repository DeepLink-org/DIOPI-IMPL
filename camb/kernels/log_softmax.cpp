...
extern "C" diopiError_t diopiLogSoftmax(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t input, int64_t dim, diopiDtype_t dtype){
    ...
    DIOPI_CALLCNNL(cnnlSetTensorDescriptor(x_desc, CNNL_LAYOUT_ARRAY, input_dtype, input_dim, input_shape.data()));
    DIOPI_CALLCNNL(cnnlSetTensorDescriptor(y_desc, CNNL_LAYOUT_ARRAY, out_dtype, input_dim, input_shape.data()));
